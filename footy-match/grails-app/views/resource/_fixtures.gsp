                ...
                <table id="resourceAllocations" class="list">
                    ...
                    <tbody>
                        ...
                        <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
                            ...
                            <td class="date">
                                <g:select onchange="previewAllocations()" name="pitch${f.id}" from="${availablePitches}" noSelection="[null:'-- Not Assigned --']" optionKey="id" />
                            </td>
                            <td class="date">
                                <g:select onchange="previewAllocations()" name="ref${f.id}" from="${availableReferees}" noSelection="[null:'-- Not Assigned --']"  optionKey="id" />
                            </td>
                            <td class="date">
                                <g:select onchange="previewAllocations()" name="chrm${f.id}" from="${availableChangingRooms}" noSelection="[null:'-- Not Assigned --']" optionKey="id" />
                            </td>
                        </tr>
                        </g:each>
                    </tbody>
